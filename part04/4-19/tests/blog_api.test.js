...
describe('when there is initially one user in db', () => {
  ...
  test('creation fails with proper statuscode and message if username/password is less than 3 characters long', async () => {
    const usersAtStart = await helper.usersInDb()
    ...
    const usersAtEnd = await helper.usersInDb()
    assert(result.body.error.includes('expected `username` and `password` to be at least 3 characters long'))
